...
# Ray
RAY_ADDRESS=${RAY_ADDRESS:-"http://0.0.0.0:8265"}
WORKING_DIR=${WORKING_DIR:-"${PWD}"}
RUNTIME_ENV=${RUNTIME_ENV:-"${WORKING_DIR}/verl/trainer/runtime_env.yaml"}
NNODES=${NNODES:-"$WORLD_SIZE"}
# Paths
RAY_DATA_HOME=${RAY_DATA_HOME:-"/global_data/med/lukai/verl"}
MODEL_PATH=${MODEL_PATH:-"/global_data/med/lukai/modelscope/Qwen/Qwen2.5-Math-7B"}
CKPTS_DIR=${CKPTS_DIR:-"/checkpoint_save/ckpts/${project_name}/${exp_name}"}
...
if [ ${RANK} == 0 ]; then
  ...
  ray job submit --runtime-env="${RUNTIME_ENV}" \
    ...
    trainer.resume_mode=disable 2>&1 | tee ${CHECKPOINT_SAVE}/run.log
