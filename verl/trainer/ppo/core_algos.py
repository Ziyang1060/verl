...
def agg_loss(loss_mat: torch.Tensor, loss_mask: torch.Tensor, loss_agg_mode: str):
    ...
    if loss_agg_mode == "token-mean":
        ...
        loss = torch.sum(seq_losses) / (loss_mask.shape[0] * loss_mask.shape[1])  # The divisor
        # Fix Bug
...
def compute_policy_loss_cov(
    ...
    # Off-Policy 训练稳定性改进 from [R1-Reward: Training Multimodal Reward Model Through Stable Reinforcement Learning]
    # Pre-CLIP: 避免log_diff过大而找到exp操作溢出，同时防止loss过大，注意此步会将token剥离计算图
    negative_approx_kl = torch.clamp(negative_approx_kl, max=np.log(1e3), min=np.log(1e-3)) 
    ratio = torch.exp(negative_approx_kl)
    # Advantage Filter: 这里的advantages已经被z-normalization了（均值为0，标准差为1），用 3-sigma 原则做截断
    ...
    # The Entropy Mechanism - Theorem 1 sample-based
    sample_log_prob = verl_F.masked_mean(log_prob.detach(), response_mask, axis=1) # bsz
    sample_prob = torch.exp(sample_log_prob)
    sample_adv_prob = verl_F.masked_mean(advantages, response_mask, axis=1) * sample_prob # bsz
    sample_cov = (sample_adv_prob - sample_adv_prob.mean()) * (sample_log_prob - sample_log_prob.mean()) # bsz
    cov_t1_sample = sample_cov.mean()
    # The Entropy Mechanism - Theorem 2 token-based
    token_cov = (advantages - verl_F.masked_mean(advantages, response_mask)) * (log_prob.detach() - verl_F.masked_mean(log_prob.detach(), response_mask))
    token_cov[response_mask == 0] = -999999
    clip_by_origin = (pg_losses2 > pg_losses1) & (response_mask > 0)
    token_cov[clip_by_origin] = -999999
    cov_t2_token = verl_F.masked_mean(token_cov, token_cov != -999999)
    ...
    return pg_loss, pg_clipfrac, ppo_kl, pg_clipfrac_lower, cov_t1_sample, cov_t2_token
