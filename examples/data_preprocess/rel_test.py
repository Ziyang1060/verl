...
data_source = "rel_tiny_knowledge_process" # "rel_tiny_random" "rel_tiny_uniform" "rel_tiny_longtail" "rel_tiny_knowledge_process"
...
inputs = (
    "<query>{query}</query><note>{note}</note><qtax>{q_tax}</qtax><ntax>{n_tax}</ntax>"
)
user_prompt_template = instruction_v4 + "\n" + inputs

df = pd.read_csv(dataset_path, sep="\t", quoting=3)
user_prompts = df.apply(
...
print(train_dataset[0])
print(len(train_dataset))
train_dataset.to_parquet(dataset_path.replace(".csv", ".process.test.parquet"))